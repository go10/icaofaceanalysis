...
class FaceWebApp:
    '''
    Use Azure for face image quality checks.
    Streamlit (https://docs.streamlit.io/) is used for the web app.
    '''

    def __init__(self):
        '''
        The Azure credentials are read from the local environment variables.
        ...
        st.set_page_config(page_title="Face Image Processing (Azure)", page_icon=":camera:")
        st.title("Face Image Processing (Azure Face)")
        self.conf_is_face = 98.0
    ...
    def run(self):
        ...
        if uploaded_file is not None:
            ...
            if st.button("Analyze"):
                ...
                if False: # temporary to force call to Azure
                    ...
                    st.write("Calling Azure Face API")
                    photo = open(save_image_path, 'rb')
                    response = None
                    try:
                        response = self.client.face.detect_with_stream(
                            image=photo)
